class User < ActiveRecord::Base
  ...
  attr_accessible :email, :first_name, :last_name, :password, :password_confirmation, :role,
                  :send_emails, :is_confirmed, :confirmation_token

  enumerize :role, in: [:demo, :user, :admin, :super_admin]
  ...
  ALL_ROLES_ARRAY = [:user, :admin, :super_admin, :demo]
  ASSIGNABLE_ROLES = { admin: [:user, :admin, :demo],
                       super_admin: ALL_ROLES_ARRAY }
  SORTABLE_COLUMNS = [:email, :first_name, :last_name, :created_at, :role, :send_emails,
                      :is_confirmed].collect { |column| column.to_s }
  SORTABLE_STRING_COLUMNS = [:email, :first_name, :last_name].collect { |column| column.to_s }
  ...
  def self.find_by_names(first_name, last_name)
    User.where("lower(first_name) = lower(?) AND lower(last_name) = lower(?)", 
